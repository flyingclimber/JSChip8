                ...
                for (var yline = 0; yline < h; yline++) {
                    ...
                    for ( var xline = 0; xline < spriteSize; xline++) {
                        if ((pixel & (0x80 >> xline)) !== 0 ) {
                            if (this.gfx[(x + xline + ((y + yline) * height))] === 1) {
                                this.v[0xF] = 1;
                            }
                            this.gfx[x + xline + ((y + yline) * height)] ^= 1; 
                        }
                    }
                }
            ...
            case 0xF000:
                switch(opcode & 0x00FF) {
                    ...
                    case 0x0030: //FX30 SuperChip8: Sprite is 10 bytes high.
                        character = this.v[(opcode & 0x0F00) >> 8];
                        this.I = parseInt(character, 16) * 10;
                        this.pc += 2;
                        break;
...
function updateGraphics() {
    ...
    for (y = 0; y < Chip8.width; y++) {
        for (x = 0; x < Chip8.height; x++) {
            if (Chip8.gfx[(height * y) + x]) {
...
function scrollScreen(down,left,right) {
    ...
    for(y = 0; y < Chip8.width; y++) {
        for(x = 0; x < Chip8.height; x++) {
            if(Chip8.gfx[(Chip8.height * y) + x]) {
                if(down) {
                    gfxCopy[((Chip8.height * y) + down) + (x + left + right)] = 1; 
                }
                if(left) {
                    gfxCopy[((Chip8.height * y) - (Chip8.height * left))] = 1; 
                }
                if(right) {
                    gfxCopy[((Chip8.height * y) + (Chip8.height * right))] = 1; 
                }
            }
        }
    }

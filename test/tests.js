...
test("0x00FB", function() {
    Chip8.initialize(['0x00','0xFB'])
    Chip8.gfx[0] = 1;
    Chip8.decodeOpcode();
    equal(Chip8.gfx[0+64*4],1,"Shift four pixels right");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x00FC", function() {
    Chip8.initialize(['0x00','0xFC']);
    Chip8.gfx[256] = 1;
    Chip8.decodeOpcode();
    equal(Chip8.gfx[1],1,"Shift four pixels left");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x00FE", function() {
    Chip8.initialize(['0x00','0xFE']);
    Chip8.decodeOpcode();
    equal(Chip8.height,64,"Change height to 64");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x00FF", function() {
    Chip8.initialize(['0x00','0xFF']);
    Chip8.decodeOpcode();
    equal(Chip8.height,128,"Change height to 128");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x0075", function() {
    Chip8.initialize(['0xF1','0x75']);
    Chip8.v[1] = 50;
    Chip8.decodeOpcode();
    equal(Chip8.flag[1], 50,"Store 50 in V[1] and move to flag[1]");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x0085", function() {
    Chip8.initialize(['0xF1','0x85']);
    Chip8.flag[1] = 50;
    Chip8.decodeOpcode();
    equal(Chip8.v[1], 50,"Store 50 in V[1] and move to flag[1]");
    equal(Chip8.pc,514,"Increment pc");
});

test("0x00CN", function() { 
    Chip8.initialize(['0x00','0xC0']);
    Chip8.gfx[0] = 1;
    Chip8.decodeOpcode();
    equal(Chip8.gfx[5],1, "Shift 5 spaces down");
    equal(Chip8.pc,514,"Increment pc");
});
